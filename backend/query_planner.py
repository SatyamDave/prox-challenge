...
class QueryPlanner:
    ...
    def __init__(self):
        ...
        self.query_patterns = {
            "duty_cycle": ["duty cycle", "duty", "continuous", "how long", "runtime"],
            "wire_speed": ["wire speed", "feed rate", "ipm", "wire feed"],
            "amperage": ["amps", "amperage", "current", "how many amps"],
            "voltage": ["voltage", "volts", "120v", "240v", "power"],
            "polarity": ["polarity", "dcep", "dcen", "electrode positive", "electrode negative", "cable", "cables", "flip", "torch", "work clamp", "reverse", "reversed", "wrong", "swap", "connection"],
            "gas": ["gas", "shielding gas", "argon", "co2", "flow rate"],
            "troubleshooting": ["problem", "issue", "defect", "spatter", "porosity", "not working", "won't"],
            "setup": ["setup", "set up", "configure", "how to", "settings", "should i use", "what do i use"],
            "material": ["steel", "aluminum", "stainless", "material", "metal"],
            "thickness": ["thick", "thickness", "gauge", "1/8", "1/4", "1/16"],
            "process": ["mig", "tig", "stick", "flux core", "process"]
        }
    ...
    def analyze_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        numbers = re.findall(r'\b(\d+(?:\.\d+)?)\s*([aA]|[vV]|amps?|volts?|inches?|mm|gauge)?\b', query)
    ...
    def create_search_plan(self, query_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def should_decompose_query(self, query_analysis: Dict[str, Any]) -> bool:
        ...
        if any(word in query_analysis["original_query"].lower() for word in ["max", "maximum", "best", "optimal", "recommended"]):
            return True

        return False

    def refine_search_queries(self, original_query: str, search_results: List[Dict], iteration: int = 0) -> Optional[str]:
        """
        ...
        if len(search_results) < 2:
            ...
            key_terms = re.findall(r'\b[A-Z]{2,}\b|\b\d+[AVav]\b|\b[a-z]{4,}\b', original_query)
            if key_terms:
